...
def design_all_primers(amr_results: dict[str, Any],
    ...
    """
    Designs PCR primers for the AMR genes AMRFinderPlus located on the assembly.

    Each template is sliced from that gene's own coordinates (plus flanks), so a
    primer pair labelled "CTX-M-15" is actually designed against the CTX-M-15
    locus. Targets without usable coordinates are reported as skipped rather
    than designed from a substitute sequence.
    """
    contigs: dict[str, str] = {}
    if assembly_fasta and Path(assembly_fasta).exists():
        for head, seq in _parse_fasta(Path(assembly_fasta)):
            contigs[head.split()[0]] = seq

    if not contigs:
        logger.warning("Primer design skipped: no assembly contigs available.")
        return []

    templates: dict[str, list[str]] = {}
    for hit in amr_results.get("genes", []):
        gene = hit.get("gene", "")
        if not gene:
            continue
        region = _extract_gene_region(contigs, hit)
        if region:
            templates.setdefault(gene, []).append(region)

    ai_genes = {t.get("gene", "") for t in ai_targets if t.get("gene")}

    results: list[dict[str, Any]] = []
    for gene, seqs in templates.items():
        res = design_primers_for_gene(gene, seqs)
        res["ai_recommended"] = gene in ai_genes
        results.append(res)
        logger.info("Primer design: %s -> %d pair(s)",
                    gene, len(res.get("primers", [])))

    # AI may suggest targets AMRFinderPlus never placed on a contig. Report
    # them explicitly instead of silently dropping or mis-designing them.
    for gene in sorted(ai_genes - set(templates)):
        results.append({
            "gene": gene,
            "n_sequences": 0,
            "conserved_region_bp": None,
            "primers": [],
            "ai_recommended": True,
            "note": "Skipped: not located on the assembly by AMRFinderPlus, "
                    "so no trustworthy template sequence was available.",
        })

    return results


FLANK_BP = 300   # flanking bases so Primer3 can place primers outside the CDS

_COMPLEMENT = str.maketrans("ACGTRYKMBVDHNacgtrykmbvdhn",
                            "TGCAYRMKVBHDNtgcayrmkvbhdn")
...
def _extract_gene_region(contigs: dict[str, str],
                          hit: dict[str, Any],
                          flank: int = FLANK_BP) -> str | None:
    """Slices a gene's own locus (plus flanks) out of the assembly.

    Returns None when the hit carries no usable contig/coordinates. Callers
    must treat that as "no template", not as a reason to substitute another
    sequence.
    """
    contig_id = (hit.get("contig") or "").split()
    if not contig_id:
        return None
    seq = contigs.get(contig_id[0])
    if not seq:
        return None

    try:
        start = int(str(hit.get("start", "")).strip())
        stop  = int(str(hit.get("stop", "")).strip())
    except (TypeError, ValueError):
        return None
    if start > stop:
        start, stop = stop, start

    lo = max(0, start - 1 - flank)
    hi = min(len(seq), stop + flank)
    region = seq[lo:hi]
    if not region:
        return None

    if (hit.get("strand") or "+").strip() == "-":
